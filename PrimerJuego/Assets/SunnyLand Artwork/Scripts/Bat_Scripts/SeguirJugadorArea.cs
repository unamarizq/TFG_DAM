...
private IEnumerator DestruirDespuesDeTiempo(float tiempo)
{
    yield return new WaitForSeconds(tiempo);
     ...
     // Probabilidad de soltar un zafiro
    if (Random.value < probabilidadSoltarZafiro) // 30% de probabilidad
    {
        Instantiate(prefabZafiro, transform.position, Quaternion.identity);
    }
    Destroy(gameObject);
}
